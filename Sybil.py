...
class Blockchain:
    def __init__(self, oneself, interval, all_node):
        ...
        self.server_socket.bind(self.all_node[self.updater])  # 绑定套接字到主机端口，address为一个元组(‘0.0.0.0’, 12345)
    ...
    def menu(self):
        while True:
            ...
            if (casual % 3 == 1):
                transfer = threading.Thread(target=self.transfer, args=self.oneself[casual%9])
                transfer.start()
                # self.transfer(self.updater)
            elif (casual % 3 == 2):
                send_message = threading.Thread(target=self.send_message, args=self.oneself[casual%9])
                send_message.start()
                # self.send_message()
            elif (casual % 3 == 0):
                change = threading.Thread(target=self.change, args=self.oneself[casual%9])
                change.start()
                # self.change()
        ...
        while True:
            ...
            if tim % self.interval == 0:
                ...
                if validator in self.oneself:
                    ...
                    self.data_send_all(message)     # 发布区块
                    print("-----sending block-----")
    ...
    def select_validator_1(self,one):
        # 按照质押金额比例随机选择验证者
        if self.balance[one] > 0:
            message = {'identifier': 30,
                       'time_stamp': time.time(),
                       'random_number': random.randint(0, 99999),
                       'sender': one
                       }
            self.random_dict[one] = message['random_number']
            self.data_send_all(message)
            print(f'{one}的数字为{message["random_number"]}')
        else:
            print("----无质押币----")

    def select_validator(self):
        for node in self.oneself:
            self.select_validator_1(node)
        print("开始挑选节点")
        ...
        # 恶意指定出块人，给出块人利息
        selected_node = self.oneself[random.randint(0, 100000)%9]
    ...
    def send_message(self,oneself):
        to_node = self.random_item(self.lists_my)
        message = {'identifier': 20,"time_stamp": time.time(), 'message': '', 'sender': oneself,
                   'recipient': to_node}
    ...
    def change(self,oneself):
        message = {'identifier': 40, 'sender': oneself, 'balance': self.balance, 'money': self.money,
                   "time_stamp": time.time()}
        self.data_send_all(message)
...
lists_all = {'A': ('127.0.0.1', 1), 'B': ('127.0.0.1', 2), 'C': ('127.0.0.1', 3), 'D': ('127.0.0.1', 4),
             'E': ('127.0.0.1', 5), 'F': ('127.0.0.1', 6), 'G': ('127.0.0.1', 7), 'H': ('127.0.0.1', 17),
             'I': ('127.0.0.1', 9), 'J': ('127.0.0.1', 10), 'K': ('127.0.0.1', 11), 'L': ('127.0.0.1', 12),
             'M':  ('127.0.0.1', 13), 'N': ('127.0.0.1', 14), 'P': ('127.0.0.1', 15), 'Q': ('127.0.0.1', 16)}
...
# lists_all = {'A': ('127.0.0.1', 12), 'B': ('127.0.0.1', 42), 'C': ('127.0.0.1', 6)}
lists_my = lists_all.copy()
Sybil  = ['H', 'I', 'J', 'K', 'L',  'M', 'N', 'P', 'Q']

for i in range(len(Sybil )):
    lists_my.pop(Sybil [i])
MAX_THREADS = len(lists_my)
lists_all = {k: lists_all[k] for k in sorted(lists_all)}  # 排序
list_keys = list(lists_my.keys())

blockchain = Blockchain(oneself=Sybil, interval=5, all_node=lists_all)
blockchain.node()
blockchain.menu()